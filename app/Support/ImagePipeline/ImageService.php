...
final class ImageService
{
    ...
    private function optimizeExact(string $absFile, Profile $profile, OptimizeOptions $opt): array
    {
        ...
        $format = strtolower($profile->format ?? pathinfo($absFile, PATHINFO_EXTENSION));
        $target = $absFile;

        if ($format === 'webp') {
            $target = preg_replace('/\.[a-zA-Z0-9]+$/', '.webp', $absFile) ?? ($absFile . '.webp');
        }

        if ($opt->dryRun) {
            return [[
                'abs' => $target,
                'rel' => $this->relFromPublic($target),
            ]];
        }

        $this->saveByFormat($img, $target, $format, $profile->quality);
