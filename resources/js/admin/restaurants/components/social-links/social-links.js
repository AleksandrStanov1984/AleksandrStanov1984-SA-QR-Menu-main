// resources/js/admin/restaurants/components/social-links/social-links.js

/*(function () {

    const app = document.getElementById('socialLinksApp');
    if (!app) return;

    const ROUTE_STORE =
        app.dataset.routeStore || '';

    const URL_UPDATE_BASE =
        app.dataset.urlUpdateBase || '';

    const REORDER_URL =
        app.dataset.routeReorder || '';

    const CSRF_TOKEN =
        app.dataset.csrf || '';

    const TITLE_ADD =
        app.dataset.titleAdd || 'Add';

    const TITLE_EDIT =
        app.dataset.titleEdit || 'Edit';
    ...
    function openEdit(payload) {
        ...
        if (payload.icon_url) {
            setIconPreview(payload.icon_url);
        }
    ...
    document.addEventListener('click', (e) => {
        ...
        if (delBtn) {
            ...
            const url =
                delBtn.getAttribute('data-delete-url');

            const text =
                delBtn.getAttribute('data-delete-text') || '';

            const hint =
                delBtn.getAttribute('data-delete-hint') || '';
    ...
    if (list && window.Sortable) {
        ...
        new Sortable(list, {
            ...
            onEnd: function () {
                ...
                const items = [
                    ...list.querySelectorAll('[data-sl-item]')
                ];
    ...
    document.addEventListener('click', (e) => {
        ...
        const run = async () => {
            ...
            try {
                ...
                const img =
                    box?.querySelector('[data-sl-icon]');
